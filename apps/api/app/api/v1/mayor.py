...
menor código, típicamente L02); si no queda ningún líder activo, pasa al
Centinela C01.
"""
...
def promote_leader_mayor(db: Session, neighborhood_id: int) -> User | None:
    ...
    Centinela C01. Devuelve el nuevo Mayor o None si no hay candidato.
    """
    if not neighborhood_id:
        return None
    ...
    if leaders:
        ...
        # No hay líder activo → usar el Centinela C01
        c01 = db.query(User).filter(
            User.neighborhood_id == neighborhood_id,
